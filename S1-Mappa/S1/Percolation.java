...
public class Percolation {
    ...
    public Percolation(int N) {
        ...
        countOfOpenSites = 0;
        Nsize = N;
        wqu = new WeightedQuickUnionUF(N * N + 2); // +2 for the top and bottom
    ...
    public boolean isFull(int row, int col) {
        validIndex(row, col);
        int site = getOneDcoordinate(row, col);
        return wqu.find(topRow) == wqu.find(site);
    }
    ...
    public static void main(String[] args) {
        ...
        test1.open(0, 1);
        test1.open(1, 2);
        StdOut.println(test1.isOpen(1, 2));
        StdOut.println(test1.isFull(1, 2));
        test1.open(1, 1);
        StdOut.println(test1.isOpen(1, 2));
        StdOut.println(test1.isFull(1, 2));
